...
OUT_PATH = BASE / 'canva-day1-7-launch-card-designs.json'

FILES = []
for day in range(1, 8):
    FILES.append((f'day-{day:02d}-instagram-4x5-launch.png', 1080, 1350, f'NE Day{day}ig'))
    FILES.append((f'day-{day:02d}-tiktok-9x16-launch.png', 1080, 1920, f'NE Day{day}tt'))
...
def main():
    token = load_token()
    results = []
    for filename, width, height, label in FILES:
        path = BASE / filename
        if not path.exists():
            raise SystemExit(f'Missing {path}')
        title = title_from_file(label)
        print('Creating', title, flush=True)
        token, asset_id = upload_asset(token, path, title)
        token, design = create_design(token, asset_id, title, width, height)
        item = {
            'file': filename,
            'title': title,
            'width': width,
