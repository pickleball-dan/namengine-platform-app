...
// ─── Standing rule: "Skip" must NEVER appear anywhere in Pet's interview ─────

test.describe('Pet — Skip must never appear', () => {
  test('No visible "Skip" text at any point during the interview', async ({ page }) => {
    ...
    for (let step = 0; step < 20; step++) {
      await expect(
        ...
        `Step ${step + 1}: found a visible element with text "Skip" in Pet interview`
      ).toBeHidden();

      if (await page.locator(V.pet.review).isVisible()) break;
      if (await page.locator(`${V.pet.q}:not([hidden])`).count() === 0) break;
      await answerAndAdvance(page, 'pet');
    }
  });
});

// ─── Pet — Card sizing must match Baby standard ───────────────────────────────
// Baby is the declared standard. Pet must match at every viewport.
...
// Pet card sizing is measured against Baby's live computed values.
// Baby is navigated first to get the reference width, then Pet is checked.
...
test.describe('Pet — Card sizing matches Baby standard', () => {
  test('Question card fills at least 65% of viewport — desktop', async ({ page }) => {
    const vp = page.viewportSize();
    if (vp.width < 1024) test.skip();

    await startInterview(page, 'pet');
    const sizing = await getCardSizing(page, V.pet.cardSel);
    expect(sizing, 'Could not read Pet card sizing').not.toBeNull();

    expect(
      sizing.width,
      `Pet card width (${sizing.width}px) must be >= 65% of ${vp.width}px viewport (${Math.round(vp.width * 0.65)}px)`
  ...
  test('Question card fills at least 85% of viewport — mobile', async ({ page }) => {
    ...
    expect(
      sizing.width,
      `Pet card width (${sizing.width}px) must be >= 85% of ${vp.width}px mobile viewport (${Math.round(vp.width * 0.85)}px)`
...
test.describe('Pet — Interview flow', () => {
  test.beforeEach(async ({ page }) => {
    await startInterview(page, 'pet');
  });

  test('Exactly one question is visible at a time after Begin', async ({ page }) => {
    await expect(page.locator(`${V.pet.q}:not([hidden])`)).toHaveCount(1);
  });
  ...
  test('Next on an empty optional text question advances without blocking', async ({ page }) => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const q = visibleQuestion(page, 'pet');
      ...
      if ((kind === 'text' || kind === 'textarea') && required === 'false') {
        ...
        await q.locator(V.pet.next).click();
        await page.waitForTimeout(500);

        // Must advance — either to next question or direction review
        const reviewNow = await page.locator(V.pet.review).isVisible();
        if (!reviewNow) {
          const afterId = await visibleQuestion(page, 'pet').getAttribute('data-question-id');
    ...
    if (!tested) {
      console.log('NOTE: No optional text question encountered in first 15 steps — verify Pet question config');
    }
  });

  test('Back returns to the previous question', async ({ page }) => {
    const q1Id = await visibleQuestion(page, 'pet').getAttribute('data-question-id');

    await answerAndAdvance(page, 'pet');

    const q2Id = await visibleQuestion(page, 'pet').getAttribute('data-question-id');
    expect(q2Id).not.toBe(q1Id);

    await page.locator(V.pet.back).click();
    await page.waitForTimeout(300);

    expect(
      await visibleQuestion(page, 'pet').getAttribute('data-question-id')
    ).toBe(q1Id);
  });

  test('Direction review appears after all questions — not results', async ({ page }) => {
    const result = await completeInterview(page, 'pet');
  ...
  test('Direction review has an Edit button for every question', async ({ page }) => {
    await completeInterview(page, 'pet');
    const allQuestions = await page.locator(V.pet.q).count();
    const editButtons = page.locator(`${V.pet.review} [data-edit-question]`);
  ...
  test('Edit from direction review navigates to that specific question', async ({ page }) => {
    ...
    const firstEdit = page.locator(`${V.pet.review} [data-edit-question]`).first();
    ...
    await expect(page.locator(V.pet.review)).toBeHidden();
  });
  ...
  test('After editing a question, direction review reappears', async ({ page }) => {
    ...
    await expect(page.locator(V.pet.review)).toBeVisible({ timeout: 2000 });
  });

  test('"Find names" shows completion screen', async ({ page }) => {
    await completeInterview(page, 'pet');
    await page.locator(V.pet.find).click();
    await expect(page.locator(V.pet.complete)).toBeVisible({ timeout: 5000 });
  });
});
